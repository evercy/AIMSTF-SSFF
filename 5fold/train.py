...
from dataset import MAHNOB, DEAP
from utils import *
...
def train20(modal, dataset, k, l, epoch_num, lr, batch_size, file_name, indices,face_feature_size,
          use_gpu, pretrain,
          withoutbaseline,
    ...
    if dataset == 'DEAP':
        ...
        train_data = DEAP(modal=modal,k=k,kind='train',indices=indices, label=l)
        val_data = DEAP(modal=modal,k=k,kind='val',indices=indices, label=l)
    if dataset == 'MAHNOB':
        ############## per-subject #################
        train_data = MAHNOB(modal=modal, k=k, kind='train', indices=indices, label=l)
        val_data = MAHNOB(modal=modal, k=k,  kind='val', indices=indices, label=l)
