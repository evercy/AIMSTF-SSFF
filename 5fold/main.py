...
def demo():
    ...
    parser.add_argument('--batch_size', '-b', default=120, help='The batch size used in training', type=int)
    parser.add_argument('--learn_rate', '-l', default=0.0005, help='Learn rate in training', type=float)
    ...
    parser.add_argument('--alpha_weight', '-alpha', default=0.01, help='diff_weight_Loss', type=float)
    parser.add_argument('--beta_weight', '-beta', default=0.5, help='sim_weight_Loss', type=float)
    parser.add_argument('--gamma_weight', '-gamma', default=0.01, help='diff_weight_Loss', type=float)
    #model
    ...
    parser.add_argument('--num_classes', default=2, help='num_layers', type=int)
    ...
    if args.dataset == 'DEAP':
        indices = list(range(52440))
    if args.dataset == 'MAHNOB':
        indices = list(range(42463))
    ...
    for k in range(1,6):
            ...
            print(
                f'{args.label}k{k}_bs_{args.batch_size}_lr_{args.learn_rate}_num_layers_{args.num_layers}'
            ...
            if not os.path.exists(
                    f'./results/{args.dataset}/{args.modal}/{args.label}_k{k}_bs_{args.batch_size}_lr_{args.learn_rate}_num_layers_{args.num_layers}_alpha_weight{args.alpha_weight}_dropout{args.dropout}/'):
                os.mkdir(
                    f'./results/{args.dataset}/{args.modal}/{args.label}_k{k}_bs_{args.batch_size}_lr_{args.learn_rate}_num_layers_{args.num_layers}_alpha_weight{args.alpha_weight}_dropout{args.dropout}/')

            best_accuracy = train20(modal=args.modal, dataset=args.dataset, k=k, l=args.label, epoch_num=args.epoch,
                                  lr=args.learn_rate, batch_size=args.batch_size,
                                  file_name=f'./results/{args.dataset}/{args.modal}/'
                                            f'{args.label}_k{k}_bs_{args.batch_size}_lr_{args.learn_rate}_num_layers_{args.num_layers}_alpha_weight{args.alpha_weight}_dropout{args.dropout}/'
